...
1. 三つの関数を作成すること．一つ目の関数は配列全体を出力する関数`print_array`．二つ目の関数は最小の要素を見つける関数`find_min`．三つ目の関数は配列の二つの要素を交換する関数`swap`．これら三つの関数をかならず使用して，下記の仕様を満たすこと．
...
```
注意点として，関数`swap`を作成する際は配列の先頭などの情報は一旦忘れて，任意の要素インデックス`x`と`y`に対してそれらの要素を交換するように実装すること．

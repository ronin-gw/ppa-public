...
表の上では，とある編集距離 ($c_{i,j}$) を計算するためには，(1) その**左上** ($c_{i-1,j-1}$)，(2) その**左隣** ($c_{i,j-1}$)，(3) その**真上** ($c_{i-1,j}$) の値が必要である．裏を返せば，これらの3つの値が計算済みであれば，それらに囲まれるマスの値を計算できるということである．例えば，$c_{2,2}$ (表の``A``) は，その左上，左隣，真上である $c_{1,1},c_{1,2},c_{2,1}$ の値を利用して，${\rm min}(c_{1,1}+1,c_{1,2}+1,c_{2,1}+1)={\rm min}($``1``$+1,$``2``$+1,$``2``$+1)=2$ と求まる．

この位置関係に着目すると，表の一行目は，左から右へ値を五月雨式に埋めることができる．さらに，一行目の値が決まれば，二行目も左から右へ値を埋めていくことができる．例えば，$c_{2,3}$ (表の``B``) は，左隣の $c_{2,2}$ (表の``A``) の計算結果を利用し，${\rm min}(c_{1,2}+1,c_{2,2}+1,c_{1,3}+1)={\rm min}($``2``$+1,$``A``$=$``2``$+1,$``2``$+1)=3$ と求まる．同様に $c_{2,4},c_{2,5},...,c_{3,1},c_{3,2},...,c_{5,7}$ と計算をしていけば全ての値が埋まり，最終的に文字列 $X$ と $Y$ の編集距離 $c_{m,n}$ を求めることができる．
...
+ コメントには、プログラムの要約や意図を記述し, インデント・行間が適切に入れられていること．
...
+ 動系計画法用の二次元配列テーブル`dpt`は，課題4-2と同じく`malloc_2d_`関数を用いてメモリを確保せよ.
...
+ 課題4-2で使用したグローバル変数は, 本課題では使用してはいけない.

---
## ステップ 1/2
---

+ 文字列$X$の位置$a$の文字と文字列$Y$の位置$b$の文字が等しい場合に0，異なる場合に1を返す関数`delta`を作成し，それを関数`dpt_delta`の中で用いること
  + 例：
  ```
...
+ 文字列$X$，文字列$X$の長さ$m$，文字列$Y$，文字列$Y$の長さ$n$，二次元配列テーブル`dpt`の5つを引数にとり，編集距離を返す関数`ld_dp`を作成すること
  + `delta`を使うこと
  + ステップ１で実装した`dpt_delta`(ステップ2では不要)を参考に, `ld_dp`を実装すること
  + 動的計画法の配列テーブルの計算は, `ld_dp`内に実装すること
  + 例：
  ```
